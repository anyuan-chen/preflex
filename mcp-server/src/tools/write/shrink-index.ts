...
async function waitForGreen(index: string, timeoutMs = 30000): Promise<boolean> {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    try {
      const health = await esFetch<{ status: string }>(
        "GET",
        `/_cluster/health/${encodeURIComponent(index)}?wait_for_status=green&timeout=5s`,
      );
...
export function registerShrinkIndex(server: McpServer): void {
  server.registerTool("shrink_index", {
    title: "Shrink Index",
    description:
      "Reduce the number of primary shards in an index. Creates a new target index with fewer shards. " +
    ...
    if (config.verifyMode === "hitl") {
      const settings = await esFetch<Record<string, unknown>>(
        "GET",
        `/${encodeURIComponent(source_index)}/_settings?flat_settings=true`,
      );
      const count = await esFetch<{ count: number }>(
        "GET",
      ...
      const opId = storePendingOperation(
        "shrink_index",
        { source_index, target_index, target_shards },
        preview,
      );
      return {
        content: [{
    ...
    const nodesResp = await esFetch<{ nodes: Record<string, { name: string }> }>(
      "GET",
      "/_nodes/_all/name",
    );
    ...
    if (nodeNames.length === 0) {
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ success: false, error: "No nodes found" }, null, 2) }],
        isError: true,
      };
    }
    ...
    try {
      await esFetch(
        "POST",
        `/${encodeURIComponent(source_index)}/_shrink/${encodeURIComponent(target_index)}`,
        {
          settings: {
