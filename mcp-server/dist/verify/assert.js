export function assertVerification(baseline, afterBenchmarks, afterDocCount) {
    const details = [];
    // --- Correctness checks ---
    ...
    for (let i = 0; i < baseline.query_benchmarks.length; i++) {
        ...
        if (!hitsMatch) {
            details.push(`Query "${before.label}" hit count decreased: ${before.total_hits} → ${after.total_hits}`);
        }
        perQuery.push({
            label: before.label,
            before_ms: before.took_ms,
    ...
    const beforeAvg = validBefore.length > 0
        ? validBefore.reduce((s, q) => s + q.took_ms, 0) / validBefore.length
        : 0;
    const afterAvg = validAfter.length > 0
    ...
    const improved = validBefore.length === 0
        ? validAfter.length > 0
