...
import { getPendingOperation, removePendingOperation, } from "./operation-store.js";
/**
...
async function executeOperation(op) {
    ...
    switch (op.action) {
        case "update_settings": {
            ...
            const before = await esFetch("GET", `/${encodeURIComponent(index)}/_settings?flat_settings=true`);
            await esFetch("PUT", `/${encodeURIComponent(index)}/_settings`, settings);
            const after = await esFetch("GET", `/${encodeURIComponent(index)}/_settings?flat_settings=true`);
        ...
        case "reindex": {
            const source = p.source_index;
            const target = p.target_index;
            const mappings = p.target_mappings;
            const settings = p.target_settings;
            const sourceCount = await esFetch("GET", `/${encodeURIComponent(source)}/_count`);
            const createBody = { mappings: { properties: mappings } };
            if (settings)
                createBody.settings = settings;
            await esFetch("PUT", `/${encodeURIComponent(target)}`, createBody);
            const reindexResult = await esFetch("POST", "/_reindex", {
                source: { index: source },
                dest: { index: target },
            });
            const targetCount = await esFetch("GET", `/${encodeURIComponent(target)}/_count`);
        ...
        case "shrink_index": {
            ...
            while (Date.now() - start < 30000) {
                try {
                    const h = await esFetch("GET", `/_cluster/health/${encodeURIComponent(source)}?wait_for_status=green&timeout=5s`);
                    if (h.status === "green") {
                        ready = true;
                        break;
                    }
            ...
            await esFetch("POST", `/${encodeURIComponent(source)}/_shrink/${encodeURIComponent(target)}`, {
                settings: {
            ...
            while (Date.now() - start2 < 60000) {
                try {
                    const h = await esFetch("GET", `/_cluster/health/${encodeURIComponent(target)}?wait_for_status=green&timeout=5s`);
                    if (h.status === "green")
                        break;
                }
                catch { /* retry */ }
...
export function registerConfirmOperation(server) {
    server.registerTool("confirm_operation", {
        title: "Confirm Operation",
        description: "Execute a pending HITL operation by its operation_id. " +
...
//# sourceMappingURL=confirm-operation.js.map