...
export function registerCancelTask(server) {
    server.registerTool("cancel_task", {
        title: "Cancel Task",
        description: "Cancel a running Elasticsearch task (e.g. a long-running search query). " +
        ...
        try {
            const result = await esFetch("POST", `/_tasks/${encodeURIComponent(task_id)}/_cancel`);
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
