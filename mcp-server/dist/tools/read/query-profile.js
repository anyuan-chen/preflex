...
export function registerQueryProfile(server) {
    server.registerTool("query_profile", {
        title: "Query Profile",
        description: "Run a search query with profiling enabled. Returns per-shard timing breakdown: " +
        ...
        const result = await esFetch("POST", `/${encodeURIComponent(index)}/_search`, { query, size, profile: true });
        return {
