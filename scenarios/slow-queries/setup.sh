...
setup_slow_queries() {
  ...
  local query_index="sb-${id}-slow-query-samples"
  ...
  queries_data+='{"name":"leading_wildcard","description":"Search using leading wildcard — forces full index scan","query_body":"{\"query\":{\"wildcard\":{\"message\":{\"value\":\"*timeout*\"}}}}"}'$'\n'
  ...
  queries_data+='{"index":{"_index":"'"$query_index"'"}}'$'\n'
  queries_data+='{"name":"unbounded_agg","description":"High cardinality terms agg with no size limit","query_body":"{\"size\":0,\"aggs\":{\"all_traces\":{\"terms\":{\"field\":\"trace_id\",\"size\":100000}}}}"}'$'\n'
